"""
    LPLGraph model
        ...
        - Variance of causal strength estimate is provided for the agent as intrinsic motivation
"""
...
# LPLgraph parameters
max_reward = float(config['max_reward'])
curiosity_weight = float(config['curiosity_weight'])
...
while True:
    ...
    for i_episode in range(batch_size):
        ...
        for t in count():
            ...
            act_confidence = graph.action_confidence(current_state_hash, action.item())
            ...
            if done:
                memory.add_transition(action, log_prob, next_state, extrinsic_reward=running_reward, intrinsic_reward=curiosity)
            else:
                memory.add_transition(action, log_prob, next_state, extrinsic_reward=0., intrinsic_reward=curiosity)
    ...
    policy_net.optimizer_model_with_curiosity(memory, batch_size, policynet_optimizer, device=device)
